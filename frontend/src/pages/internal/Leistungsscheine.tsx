...
export default function Leistungsscheine() {
  const { data: scheine, isLoading } = useQuery({
    queryKey: ['intern', 'leistungsscheine'],
    queryFn: api.intern.leistungsscheine.list,
  })
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading || !scheine ? (
        ...
        <p className="text-sm text-slate-500">Keine Leistungsscheine vorhanden.</p>
      ) : (
        <div className="bg-slate-900 border border-slate-800 rounded-xl divide-y divide-slate-800">
          {scheine.map(ls => (
            <Link
              key={ls.id}
              to={`/intern/leistungsscheine/${ls.id}`}
