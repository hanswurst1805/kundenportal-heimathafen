...
export default function AngebotErstellen() {
  ...
  const { data: anfrage, isLoading } = useQuery({
    queryKey: ['intern', 'anfragen', id],
    queryFn: () => api.intern.anfragen.get(id!),
    enabled: !!id,
  })

  const [titel, setTitel] = useState('')
  const [gueltigBis, setGueltigBis] = useState('')
  const [positionen, setPositionen] = useState<PositionRow[]>([{ bezeichnung: '', menge: '1', einzelpreis: '0' }])

  const create = useMutation({
    mutationFn: () =>
      api.intern.anfragen.createAngebot(id!, {
  ...
  if (isLoading || !anfrage) return <p className="text-sm text-slate-500">Lade…</p>
  ...
  return (
    <div className="max-w-2xl space-y-4">
      <h1 className="text-xl font-semibold text-white">Angebot erstellen</h1>
      <p className="text-sm text-slate-400">Für Anfrage {anfrage.anfrage_nr} – {anfrage.thema}</p>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4">
        ...
        <button
          ...
          disabled={!titel || create.isPending}
